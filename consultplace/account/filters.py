...
from .models import (Student, UserInterestsFirst, UserInterestsSecond,
                     UserInterestsThird)


class StudentFilter(django_filters.FilterSet):
    interest_first = django_filters.CharFilter(
        field_name='interest_first__name',
        lookup_expr='icontains'
    )
    other_interest_first = django_filters.CharFilter(
        field_name='other_interest_first',
        lookup_expr='icontains'
    )

    interest_second = django_filters.CharFilter(
        field_name='interest_second__name',
        lookup_expr='icontains'
    )
    other_interest_second = django_filters.CharFilter(
        field_name='other_interest_second',
        lookup_expr='icontains'
    )

    interest_third = django_filters.CharFilter(
        field_name='interest_third__name',
        lookup_expr='icontains'
    )
    other_interest_third = django_filters.CharFilter(
        field_name='other_interest_third',
        lookup_expr='icontains'
    )
    ...
    def queryset(self, request, queryset):
        if self.value():
            return queryset.filter(
                Q(interest_first__id=self.value())
                | Q(other_interest_first=self.value())
            )
...
class InterestSecondFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        if self.value():
            return queryset.filter(
                Q(interest_second__id=self.value())
                | Q(other_interest_second=self.value())
            )
...
class InterestThirdFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        if self.value():
            return queryset.filter(
                Q(interest_third__id=self.value())
                | Q(other_interest_third=self.value())
            )
...
class ProjectsCountFilter(admin.SimpleListFilter):
    ...
    #             projects_completed=Count('group__project')
    #         ).filter(projects_completed=1)
    #     if self.value() == '2':
    #         return queryset.annotate(
    #             projects_completed=Count('group__project')
    #         ).filter(projects_completed=2)
    #     if self.value() == '3':
    #         return queryset.annotate(
    #             projects_completed=Count('group__project')
    #         ).filter(projects_completed__gte=3)


class ProjectsInProgressFilter(admin.SimpleListFilter):
    title = 'Projects in Progress'
    parameter_name = 'projects_in_progress'
...
class TasksInProgressFilter(admin.SimpleListFilter):
